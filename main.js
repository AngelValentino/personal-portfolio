...
const projectsData = [
  {
    ...
    imageUrl: 'images/webpages-screenshots/racing-spirit.jpg',
  ...
  },
  {
    ...
    imageUrl: 'images/webpages-screenshots/youtube-clone.jpg',
  ...
  },
  {
    ...
    imageUrl: 'images/webpages-screenshots/taskflow.jpg',
...
//TODO Add toggle more project info logic
  //TODO Open modal
  //TODO Close modal
  //TODO Add toggle modal events reusable modal function


function generateProjectLinkHTML(projectData) {
  return `
    <a class="project__img-link" aria-label="Go to ${projectData.title} live demo." target="_blank" href="${projectData.demoUrl}">
      <img aria-hidden="true" role="presentation" class="project__img" src="${projectData.imageUrl}" alt="${projectData.title} screenshot">
...
function generateProjectTechIconsHTML(techTitles) {
    // Filter objects based on techTitlesArray
...
function generateProjectInfoHTML(projectData) {
  return `
    <div class="project__info">
      <h3 class="project__info-title">${projectData.title}</h3>
      <p class="project__info-summary-text">${projectData.description}</p>
      <ul class="project__technologies-list">
        ${generateProjectTechIconsHTML(projectData.technologies)}
      </ul>
      <div class="project__links-container">
        <a aria-label="Go to ${projectData.title} code repository." class="project__link project__code-link" target="_blank" href="${projectData.codeUrl}">
          <h4 class="project__link-title">Code</h4>
        ...
        <a aria-label="Go to ${projectData.title}  live demo." class="project__link project__demo-link" target="_blank" href="${projectData.demoUrl}">
          <h4 class="project__link-title">Live Demo</h4>
...
function generateProjectHTML(isMirrowed, projectData) {
  return `
    <li class="project ${!isMirrowed ? 'original' : 'mirrowed'}">
      <button title="More info" class="project__more-info-btn">
        <svg class="project__more-info-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
      ...
      <button class="project__info-close-btn">
        <svg class="project__info-close-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 6L6 18M6 6l12 12" />
        </svg>
      </button>
      ${!isMirrowed ? generateProjectLinkHTML(projectData) : generateProjectInfoHTML(projectData)}
      ${!isMirrowed ? generateProjectInfoHTML(projectData) : generateProjectLinkHTML(projectData)}
    </li>
  `;
}

projectListLm.innerHTML = projectsData.map((project, i) => {
  if (i % 2 === 0) {
    return generateProjectHTML(false, project)
  } 
  else {
    
    return generateProjectHTML(true, project)
  }
